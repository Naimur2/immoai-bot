...
interface ChatToolbarProps {
    onSend: (data: { message: string; media: Media | false, file_id : any }) => void;
    isDisabled: boolean;
}
        ...
        <div className="px-4 w-full bg-white">
            ...
            >
                ...
                <label className="cursor-pointer">
                    <input type="file" accept={allowedTypes.join(",")} className="hidden" onChange={handleFileChange} />
                    <DocumentUpload
                    ...
                    {media && (
                        <div className="flex items-center gap-2 p-2 border rounded-md">
                            {media.file.type.startsWith("image") ? (
                                <img src={media.preview} alt="Preview" className="w-10 h-10 object-cover rounded" />
                            ) : (
                                <span className="text-sm text-gray-700">Document: {media.file.name}</span>
                            )}
                            <button onClick={removeMedia} className="text-red-500 text-xs">Remove</button>
                        </div>
                    )}
                    <textarea
